...
针对北京交通大学 MIS 课程平台（JYD 教育云，`http://123.121.147.7:88/ve`）的 Chrome 扩展，支持批量扫描和下载所有课程的电子课件。
...
1. **Content-Disposition 响应头**（优先级最高）  
   解析 `filename*=UTF-8''...`、`filename*=GBK''...`、`filename="..."` 等多种格式，含 `decodeURIComponent` 容错
...
4. **魔数检测**（兜底）  
   发送 `Range: bytes=0-2047` 读取文件头 2 KB，识别：
   - `%PDF` → `.pdf`
   - `\xFF\xD8\xFF` → `.jpg`
   - `\x89PNG` → `.png`
   - `Rar!` → `.rar` / `7z\xBC\xAF` → `.7z` / `\x1F\x8B` → `.gz`
   - `\xD0\xCF\x11\xE0`（OLE2）→ `.ppt`（老版 Office）
   - `PK\x03\x04`（ZIP）→ 扫描 ZIP 本地文件头中的明文目录名：
     - 含 `word/` → `.docx`
     - 含 `ppt/` → `.pptx`
     - 含 `xl/` → `.xlsx`
     - 其他 → `.zip`
...
**原因**：  
- 平台 API 的 `RP_PRIX` 字段对许多文件返回字符串 `'undefined'`（而非空值，需特判）
- `rpName` 文件名本身不含扩展名（如 `Lecture_1_Ch01_Introduction_1`）
- 下载代理 URL 以 `.shtml` 结尾，路径提取逻辑正确排除了它，但也因此无法获得扩展名

**解决方案**：  
建立上述 4 级优先检测链，魔数检测作为最终兜底，扫描阶段同步补充 `fileType` 来源。

#### 问题二：DOCX 被误识别为 PPTX

**原因**：  
DOCX、PPTX、XLSX 均以 `PK\x03\x04`（ZIP 格式）开头，仅凭文件头 4 字节无法区分，早期代码统一猜 `.pptx`。

**解决方案**：  
将魔数检测的 Range 从 8 字节扩展到 2048 字节。ZIP 格式的本地文件头（Local File Header）以明文存储目录名，2 KB 的范围足以覆盖前几个文件头，其中必然包含 `word/`（DOCX）、`ppt/`（PPTX）或 `xl/`（XLSX）等特征路径，扫描这些字符串即可精确区分三种格式。

#### 问题三：URL 路径误识别服务端脚本后缀
...
#### 问题四：Service Worker 重启后 `pageTabId` 丢失
...
#### 问题五：GBK 编码导致课程平台 sessionId 难以提取

**原因**：  
平台页面使用 GBK 编码，`sessionId` 通过服务端渲染内嵌在页面 JS 脚本中，直接读取 DOM 可能因编码问题失败。

**解决方案**：  
通过正则从 `document.scripts` 中匹配 `setRequestHeader('sessionId', 'XXXX')` 模式，直接读取原始字符串，绕开编码转换问题。
...
- 受限文件（`stu_download != '2'`）无法下载，插件不会尝试绕过
